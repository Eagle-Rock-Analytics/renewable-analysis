...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Calculate daily Standardized Energy Index (SEI) for renewable energy data with batched computation."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Number of days to compute per batch (default: 30)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
...
def main():
    ...
    print("=" * 60)
    print("SEI Calculation Parameters (Batched):")
    print("=" * 60)
    ...
    ## Compute SEI for Each Day-of-Year with Batched Processing
    print(f"\nComputing SEI for each day-of-year in batches of {batch_size}...")
    print("This may take several minutes...")

    # Process in batches to avoid massive dask graph buildup
    sei_batches = []
    gen_batches = []
    ...
    for doy in range(1, 366):
        ...
        # Compute batch when batch_size is reached or on last day
        if doy % batch_size == 0 or doy == 365:
            print(f"Computing batch through day {doy}/365...")
            sei_batch = xr.concat(sei_list, dim="dayofyear").compute()
            gen_batch = xr.concat(gen_list, dim="dayofyear").compute()

            sei_batches.append(sei_batch)
    ...
    # Concatenate pre-computed batches (fast since already in memory)
    sei_full = xr.concat(sei_batches, dim="dayofyear")
    gen_full = xr.concat(gen_batches, dim="dayofyear")
    ...
    drought_ds.attrs["batch_size_days"] = batch_size
    drought_ds.attrs["method"] = (
        "memory-efficient dynamic window extraction with apply_ufunc parallelization and batched computation"
    )
